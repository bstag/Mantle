...
const LogoCard: React.FC<LogoCardProps> = ({
  ...
  const filename = logoType === 'primary' ? 'Primary_Sigil' : 'Secondary_Crest';

  const handleVectorize = async () => {
    if (!logo) return;
    setIsVectorizing(true);
    try {
      await vectorizeAndDownload(logo, filename);
    } catch (error) {
    ...
    <div className="bg-surface rounded-2xl p-6 border border-dim flex flex-col items-center backdrop-blur-sm relative group/card">
      ...
      {logo && (
        <div className="flex flex-wrap items-center justify-center gap-2 mt-4">
          <button 
            onClick={() => downloadImage(logo, `${filename}.png`)}
          ...
          <button 
            ...
            className="px-4 py-2 bg-accent/10 hover:bg-accent/20 text-accent text-xs rounded-full transition-opacity flex items-center gap-2 disabled:opacity-50"
            title="Convert to vector SVG format"
          >
            {isVectorizing ? (
